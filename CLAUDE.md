...
- **このリポジトリは GitHub 公開・AGPL-3.0**(依存の PyMuPDF が AGPL v3.0 / 商用の二重ライセンスで、本リポジトリは AGPL 側を選択しているため)。実 tailnet ホスト名・DSN・API キー・書籍 PDF をコミットしない(過去に実ホスト名の混入をレビューで差し戻した実績あり)。依存追加は AGPL-3.0 と両立するライセンスに限る
